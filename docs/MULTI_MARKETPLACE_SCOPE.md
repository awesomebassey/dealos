...
## Domain isolation
Every document belongs to a listing and is accessed through an explicitly authorized deal. Buyers can submit their own seller questions during diligence, and the advisor can also generate evidence-linked questions from the structured review. Exact repeated buyer questions are replay-safe. Every diligence finding, question, offer, escrow account and asset-transfer item belongs to a deal. All UX routes after starting an acquisition carry the actual deal ID. Global navigation shows deal selection, never automatically selects the first result. Sellers own listings through an organization membership, and buyers only see published listings.

## Persistence and concurrency
Use PostgreSQL/Prisma transactions for offers, stage changes, publication and simulated financial movements. Lock the shared listing row when starting acquisitions, submitting offers, accepting or declining offers, changing evidence and publishing so buyers cannot enter an accepted or unverified listing in a race. Every simulated wallet top-up and escrow transfer accepts an idempotency key and writes the result in the same transaction as its ledger movements. Conditioned balance decrements prevent overspending under concurrent requests. Existing escrow ledger remains append-only. External side effects use the transactional outbox. A deal stage change keeps optimistic version checks; offers must be accepted through a single-winner database constraint.

## Verification and files
Personal: any ONE of NIN slip, BVN confirmation, driver's license, voter card. Each seller listing independently requires fabricated CAC registration, ownership and business-address evidence, plus bank statement and profit/loss samples. A newly created listing starts pending even when the same seller has other approved businesses. Replacement evidence archives a published listing until approval and explicit republication; an accepted or sold listing cannot change verification evidence mid-transaction. A seller's new personal identity submission also pauses their currently published listings pending re-approval. Demo uploads are private, tied to the relevant account or listing, and limited by MIME type and size. Small synthetic files and their metadata are stored in PostgreSQL binary columns rather than on ephemeral API disks; real sensitive documents would require a different production storage architecture. Demo approval is a separate advisor action, visibly labelled simulated; no real identity verification. Never upload real government IDs or bank statements to public previews or seed fixtures. For production, replace demo storage with encrypted private object storage, malware scanning and defined retention/erasure policies.
